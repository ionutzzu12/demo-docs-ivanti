...
1. Go to the **Admin>Windows Updates**. The following details of the updates are displayed in the page.
   ...
   **Classification**: Classifies the type of update into one of the following categories:
   ...
   - Upgrade
   - Update Rollups
2. **Distribution**: The distribution performed for the update. For example, it display **All** when the update is distributed to all the devices.
...
3. In addition, you can view if an update has been distributed to the required devices or not. The following columns have numbers that indicate the number of devices present under different category of the updates: 
   - Eligible devices
   - Installed devices
   - Failed devices
   - Reboot pending devices
4. When you click on any of these numbers, you will be directed to the filtered view on the Devices page to know the status of updates and perform the required actions.
...
5. Under **Actions**, click **Set Distribution**.
6. In the **Distribute Windows Update** window, select any of the following distribution options:
...
1. In the **Windows 10 Update Management** page, type the Knowledge base ID in the quick search field(only the number in the Search field.
