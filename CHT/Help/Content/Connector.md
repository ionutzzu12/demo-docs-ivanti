...
2. Use the **Logging Level** pulldown menu to choose a level.
   ...
   - Debug
   - Trace
3. The Info level is the default logging level setting. If you choose another logging level a rotating Sync icon ![](Sync_icon.png) appears indicating that information is being collected at the level of logging that you selected. The logging level will reset to the Info level after an hour. The Trace level is the highest logging level setting. Use this level to collect all the messages at all the other levels. The sync icon is displayed for the duration of the request.
4. If needed, hover over the Sync icon ![](Sync_icon.png) to see the Cancel icon ![](Cancel_icon.png) . Click the Cancel icon to cancel the logging level change.
5. Hover over the Request icon to display the Request information. Click the Request icon ![](Fetch_icon.png) to request the files from the current log folder in a .zip file.
...
6. If needed, hover over the Request icon ![](Fetch_icon.png) and it becomes the Cancel icon ![](Cancel_icon.png) . Click the Cancel icon to stop the request.

   When a request is canceled before completion, the Download icon ![](Download_icon.png)  is not displayed because the previous log .zip file was deleted from the server. The original log files on the Connector are still available to request.
7. Click the Download icon ![](Download_icon.png) when the request is completed to download the log .zip file containing log files collected during the latest request.
...
8. Optionally, use the **Actions** pulldown menu to Disable or Remove the Connector.

If you cannot see the **Connector** page, it might be that you do not have the required permissions. You need one of the following [**roles**](<./User Roles.md>):

- System Management
- System Read Only

For more information, see [**Using the httproxy command for Connector**](<./Using the httpproxy command for Connector.md>).
