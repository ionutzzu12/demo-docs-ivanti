...
- During the configuration of the local or external certificate authority, select the **Cache Identities on&#x20;**&#x6F;ption to store certificates with the service. Clear cache to generate certificates each time as needed.
...
### Enabling On-Premise SCEP certificate generation in batches

You can configure to define the bath size for multiple certificates in batches at fixed intervals for certificate generation request.

**Procedure**

1. Under the **Certificate Authority** section in the **Certificate Management** page, select **+ Add** > **Connect to an on-premises SCEP Certificate Authority** to create a new on-premises SCEP Certificate Authority or select **Actions** > **Edit** to modify an existing on-premises SCEP Certificate Authority.
2. Select the **Enable Batch Processing** checkbox to process certificates in batches.
3. In the **Batch Size** field, enter the number of certificates that processes in a batch.
4. In the **Batch Interval** field, enter the time duration in seconds to define the fixed intervals.
5. Click **Save**.
   To avoid certificate generation failure, allocate sufficient time duration to process a batch depending on the system load.
...
For more information, see [**SCEP configuration for external certificate authorities**](<./SCEP Configuration for External Certificate Authorities.md>).
