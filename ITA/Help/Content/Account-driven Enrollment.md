...
- A user account in with managed Apple ID (users in Apple Business Manager or Apple School Manager).
...
1. The user enters username\@acme.com to sign in to their work or school account, then the device makes a HTTP GET request call to the URL:
   ...
   For more information, see - https\://developer.apple.com/documentation/devicemanagement/discover\_authentication\_servers
...
3. Set a default assignment for **Mac**, **Apple Vision Pro**, **iPhone**, and **iPad** platform types.
4. On the console, navigate to **Admin** > **Apple** > **Device Enrollment**. In the **Device Enrollment** screen, **ADDE URL** and **ADDE URL SET TIME** columns are added.
   **First Preference**: The device checks the well-known HTTP resource at the organization’s domain.
   **Fallback Option**: If the first method fails, the device contacts Apple ABM/ASM to fetch the alternative destination specified by .
