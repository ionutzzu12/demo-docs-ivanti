...
- Configure at least one MDM server from the Apple Business Manager or Apple School Manager organizations, where managed apple IDs are present in the .
...
The devices will fail to log in if the **Managed Apple** account is not present in the Device Enrollment MDM server.
