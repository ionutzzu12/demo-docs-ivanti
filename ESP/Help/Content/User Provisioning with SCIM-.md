...
- Configuring Attributes in SCIM Provisioning

## [****](#)User Provisioning

User Provisioning uses the SCIM protocol to synchronize an IdP with and allows for partial user and group sync. User Provisioning uses the SCIM protocol to automatically create and update user and group objects sourced from an IdP to Ivanti Neurons for MDM. Just like the current integration with Entra ID and Okta, the users and groups provisioning process is automated; if changes are made to the user or group in IdP, the same changes are reflected in Ivanti Neurons for MDM.

Since the username value is unique in , the User Principal Name attribute cannot be updated in IdP if the user is already provisioned.

SCIM users and groups can now be searched using their respective external Ids.

Usernames registered in SCIM can be renamed now.

## [****](#)Generate a token from

To start User Provisioning for an IdP, generate a token and the target URL from .
...
After the connection is established, you can map the attributes between IdP and . supports the following user attributes:
