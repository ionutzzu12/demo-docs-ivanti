...
1. Activating Bridge licenses
2. Installing the Bridge application
3. Uploading scripts for permanent or one time use to the devices
...
|                  | Script File                        | Select a valid script or executable file (.ps1, .reg, .exe).<br />* The specified script file or executable file (.ps1, .reg, .exe) will be automatically executed.
* Other file types will only be copied to the target folder. |
|                  | Script Arguments                   | Specify the list of arguments for the script file.<br />* For Win32 (.exe) files, enter an argument to silently run the file (for example, /SILENT or /VERYSILENT). This is mandatory.                                           |
|                  | Target Folder                      | Specify the target folder for the script file.<br />* If the target folder is not specified, then the value of the %TEMP% system environment variable is used as the target folder.                                              |
| Undo Script File | All Versions (Windows 10+ Desktop) |                                                                                                                                                                                                                                  |
|                  | Script File                        | Select a valid script or executable file (.ps1, .reg, .exe).<br />* The specified script file or executable file (.ps1, .reg, .exe) will be automatically executed.
* Other file types will only be copied to the target folder. |
|                  | Script Arguments                   | Specify the list of arguments for the script file.<br />* For Win32 (.exe) files, enter an argument to silently run the file (for example, /SILENT or /VERYSILENT). This is mandatory.                                           |
|                  | Target Folder                      | Specify the target folder for the script file.<br />* If the target folder is not specified, then the value of the %TEMP% system environment variable is used by default.                                                        |
...
3. Click the ![](More_icon.png) icon and click **Script and Actions via Ivanti Bridge**.
4. Enter a name.
...
This feature allows you to pull Ivanti Bridge logs for individual devices for troubleshooting and diagnosing applications. The logs are sent at the next device check-in. You can wait for the next scheduled sync or perform a forced device check-in to get the logs quickly. The alternate option to pull device logs in case of an issue with Ivanti Bridge, is to use the DiagnosticLog CSP and push the logs to a cloud storage URL. You can download the Ivanti Bridge logs from the cloud storage URL for analysis.

To pull logs from a device:

1. Go to **Devices > Devices**.
2. Click the device name link to go to the Device details page. This is a Windows 10 desktop device to which the one-time script will be pushed/executed.
3. Click the ![](More_icon.png) icon and click **Pull Ivanti Bridge Log**. The **Pull Ivanti Bridge Log** window is displayed.
...
To pull logs from cloud storage URL:

1. Go to **Devices > Devices**.
2. Click the device name link to go to the Device details page. This is a Windows 10 desktop device to which the one-time script will be pushed/executed.
3. Click the ![](More_icon.png) icon and click **Request Debug Logs**. The Ivanti Bridge Logs window appears.
4. Enter the cloud storage URL in the **Share Access Signature URL** box and click **Confirm**.
5. A confirmation window appears on the screen with a message, "Pull log action has been received and is currently queued."
   After the device check-in is done, the sends a request to the device and awaits for confirmation on the status of the request. If the request is successful, the message "Logs pulled via MDM now available on the Cloud Storage" appears under the Bridge Logs section. Similarly, for an unsuccessful request, the error message appears under the Bridge Logs section.
   The administrator can access the device logs from the cloud storage location. Accessing the cloud storage location is dependent on the chosen storage option. In the case of Microsoft Azure, go to **Home** > **Storage accounts** > \{defined storage account} > **Containers** \{defined container}.
   Click **Pull Log**. After a device has sent it to Ivanti Neurons for MDM, you can view the Bridge log from the Logs tab in the Device details page.
   Only logs sent using **All logs** option can be downloaded as zip file only.
