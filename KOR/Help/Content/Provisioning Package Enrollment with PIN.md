...
The admin can enroll the devices managed by SCCM or Ivanti Endpoint Manager to the . The Deployment Package Tool allows organizations to streamline the transition of Windows devices to Modern Management, without downtime or end user interruption. The seamless transition is achieved by downloading a unique deployment package from the Console, then deploying it through the existing management tool or domain. Once the package is executed, it will silently enroll the endpoint into for the ongoing management. The approach allows administrators to first easily migrate the devices, then have the flexibility to configure devices later over-the-air. When a device completes the silent enrollment into , it is joined with MDM and is co-managed by the two management authorities. Once an administrator has configured the desired Windows experience within , the legacy management platform can be decommissioned, leaving as the single management authority of the device.

There is an exception to this rule if a device is being transitioned from Microsoft Endpoint Manager (MEM) or formerly SCCM. The existing MEM Client continues to function in Coexistence Mode (opposed to Co-Management mode), until the MEM platform is decommissioned. When Coexistence Mode is enabled, the MEM Client automatically disables certain functionality in favor of providing those workloads. For more information, see [**Microsoft's Coexistence documentation**](https://docs.microsoft.com/en-us/mem/configmgr/comanage/coexistence/).

For more exact behaviours when using MEM and other 3rd Party Management platforms, Ivanti suggests to first test the Deployment Package Tool in your environment.

**Prerequisites**

- User accounts must be imported on using LDAP, Entra ID (Entra ID), Local User Upload, or other identity integrations
...
- Enable PIN based registration on
...
- The main elements of Modern Windows Management Suite does not require Entra ID. Co-management or coexistence may require certain workloads / configurations to be deployed upon silent enrollment, to avoid any impact during transition.
...
5. The devices are enrolled on .

### Enrolling SCCM devices to

**Procedure**

1. Download all the deployment related files from the for selected users.
2. Select accounts or groups that should be enrolled.
3. Deploy the Package files to client devices using SCCM:
   1. Verify if the required clients are present in SCCM. If the Windows-configuration-designer is not present on the client, the admin should push the designer and deploy it on the client.
...
4. Execute the script to enroll a device:
   1. Go to **Software Library > Scripts** and create script.
...
4. Create PowerShell package:
   1. Verify if the required clients are present in the Endpoint Manager.
   ...
   3. Create Package: **Distribution > Distribution packages > New > Windows > PowerShell**.
...
5. Create Scheduled task:
   1. Select the created package, right-click and select **Create Scheduled task(s)**. A Scheduled task is created.
...
6. To verify the device enrollment, **Settings** > **Add or remove a provisioning package** > **Details**. Alternatively, the admin can verify a device enrollment under the Diagnostic Logs of the device.
