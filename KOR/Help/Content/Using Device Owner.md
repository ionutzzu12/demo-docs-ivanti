...
Starting with a new or factory-reset device, use the [**Provisioner**](<./Setting up the Provisioner app.md>) app to provision device owner mode using one of the following options:
...
| Item                                               | Version                                                                                                                                                                                      |
| -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Android OS<br />(on device to be provisioned)      | - 5.0 - or supported newer versions is required, if using NFC.
- 7.0 - or supported newer versions is required is required if using QR code.<br />Device must be Android Enterprise capable. |
| Android OS (on master device)                      | 5.1 - through the most recently released version.<br />Device must have NFC for using NFC bump. It is not required for QR code.                                                              |
| UEM server product, enabled for Android Enterprise | One of the following:<br />Ivanti Neurons for MDM, or Allow-labeled Ivanti Neurons for MDM.                                                                                                  |
...
1. In theportal, Go to **Configurations**.
2. Click **+Add**.
3. Select **Lockdown & Kiosk: Android Enterprise**.
4. The **Create Lockdown & Kiosk: Android Enterprise Configuration** page is displayed.
   Enter a configuration name and description.
5. Choose a Lockdown type.
   Click **Work Managed Devices (Device Owner)**.
6. Android Device Owner Lockdown settings options are displayed.
...
7. Under **Enable/Disable System Apps** section, you can optionally choose to enable to disable the following System Apps:
...
7. Optionally choose to enable **Kiosk Mode**.
...
8. Optionally select the default or custom branding options from the drop-down list.
9. Optionally create a Kiosk Exit Pin to use to exit Kiosk mode.
10. Optionally create a Allowlist of apps that will be available to users in Kiosk Mode.
