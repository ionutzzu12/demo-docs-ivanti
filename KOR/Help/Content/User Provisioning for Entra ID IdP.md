...
- Migration consideration - Entra ID User Source to User Provisioning SCIM
- Migration consideration - LDAP to User Provisioning SCIM
...
9. Click **Save**.
10. Expand **Mappings** from the **Provisioning** page on the Entra ID portal.
11. Click **Provision Entra ID Users**. The Attribute Mapping page opens.
12. Click **Delete** to delete the unsupported attributes.
...
6. Set **Managed Apple ID** - This option is disabled by default. Click the toggle button to enable it. Managed Apple ID can be set during provisioning via:
   - **User email address**
     - (Optional) select the Include "appleid" subdomain option to avoid conflict with existing Apple IDs.
   - Mapping to **Custom Attribute**
     Custom attribute should evaluate to unique email address.
...
10. Add the schema for the Custom/ Enterprise (Department) attribute as follows:
    Custom attribute - **urn\:ietf\:params\:scim\:schemas\:extension\:ivanti:2.0\:User:\<custAttr1> -&#x20;**&#x52;efer to step 3.
...
18. Log in to the administrative portal.
19. Go to **User** > **Users**> Select the user.
20. Click the **Attributes** tab, and verify the attribute value. The attribute is mapped for the specific user.
