...
5. In the Configuration Setup section, type the Carrier URL in Server Name textbox (in FQDN format).
   - Is Discovery Server - Select to mark the server as discoverable.
...
- When an enrolled device is wiped & re-enrolled; the old record of this device (with the status as Wiped) should be deleted from the Devices page to avoid provisioning issues with the eSIM configuration, onto this re-enrolled device.
