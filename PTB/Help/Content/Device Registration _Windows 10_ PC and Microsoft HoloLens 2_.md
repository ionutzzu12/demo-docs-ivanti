...
- Windows Autopilot
- With SCCM and Ivanti EPM via a Provisioning Package Enrollment with PIN. See [**Provisioning Package Enrollment with PIN**](<./Provisioning Package Enrollment with PIN.md>).
...
For more information, see Configuring Windows Autopilot.

For security and unauthorized use of the device, all Autopilot Windows devices can be locked to a tenant using the TenantLockdown CSP feature. To use this feature, the devices must be enrolled using the Autopilot option. This configuration is applied at the device level. See TenantLockdown CSP.
