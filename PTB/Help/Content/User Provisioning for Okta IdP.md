...
After the connection is established between **Okta** and , you can provision users or groups.
...
After an assigned user is provisioned on the Okta portal, verify the user provisioning on administrative portal.
...
3. Set **Automatically invite users imported from Okta** - Manage whether users imported from Okta to are automatically invited to register via email.
...
5. Click **Save****&#x20;Changes** after modifying the Okta settings.
...
5. Click on **SCIM 2.0 Test App** **(OAuth Bearer Token) User** app.
6. Click **Add Attribute**.
7. Add the following custom attribute values, for example: 
...
14. Under the **Assignments** tab, click **Assign** and select **Assign to People**.
...
To verify the custom attribute and its mapping in :
