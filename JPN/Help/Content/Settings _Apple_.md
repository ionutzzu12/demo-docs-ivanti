...
管理者は、MDMプロファイルがインストールされていない場合、エンドユーザへのメール送信、macOSおよびGo for iOSクライアントへの通知の送信を有効化または無効化することができます。 MDMプロファイル通知機能はデフォルトで有効になっています。

手順

1. ****[管理] > [設定] を開きます。
2. **[MDMプロファイルがインストールされていない場合、ユーザにメールを送信し、クライアントに通知を送信する]** オプションを選択または選択解除します。
...
管理者は、自動デバイス登録についてiOSオペレーティングシステム更新をオンにすることができます。 [****](software_Updates.htm)このオプションが有効化されている場合、Device Enrollmentデバイスは、Device EnrollmentプロファイルのOS更新のスケジュール設定ではなくソフトウェア更新の構成を使用します。 
...
管理者は、ユーザがiOS共有デバイスでマルチユーザセキュアサインのWebクリップからログアウトしたときにデバイスパスコードをクリアできます。これには [管理] > [Apple] > [設定] の「**マルチユーザセキュアサインイン**」セクションにある「**ユーザのログアウト後にパスコードをクリア**」オプションを選択します。 
...
4. 優先度を有効化した後、制約への変更は承認されるまで処理されません。 管理者は、承認の前に **[構成]** セクションから制約構成の配布、配布の概要、優先度を編集できます。
5. 優先度を有効にするには **[承認]** オプションを選択します。
6. ********iOS制約構成オプションの選択を解除するときは承認オプションが表示されません。変更は即座に適用されます。
