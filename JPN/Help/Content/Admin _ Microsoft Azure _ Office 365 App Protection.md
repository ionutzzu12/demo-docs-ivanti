...
- 1つ以上のOffice 365アプリ。
- Active DirectoryユーザのEntra IDとの同期。
- Word、Excel、PowerPointのデータを保護するためのOne Drive for Businessのデバイスへのインストール。 これは必須ではありません。
- [****](https://portal.azure.com/)Intuneアプリ保護ポリシーを構成する場合は、Microsoft Azureポータル（https://portal.azure.com/）にアクセスしてください。
- AndroidデバイスへのIntune Company Portalアプリのインストール。

   デバイスユーザによるサインインは必要ありませんが、デバイス上のデータを保護するためにこのアプリをデバイスにインストールする必要があります。 ユーザがアプリにサインインすれば、保護が自動的に適用されます。
...
Microsoftは時折、Azureポータルのユーザインターフェイスを変更する場合があります。 この手順はユーザがMicrosoft Azureポータルの使用に慣れ、AzureアプリとしてMobileIronを登録する際に必要な修正を加えられることを前提とします。

1. 最初のブラウザを開き、Microsoft Azureポータル（[**https://portal.azure.com/**](https://portal.azure.com/)）にログインします。
...
4. 以下の情報を入力し、MobileIronをAzureアプリとして登録します。
   ...
   - **サインオンURL：**デバイスユーザがMobileIronにサインイン（必須）するためのURLを入力します。
...
5. ****アプリケーションシークレット（クライアントシークレット）を管理ポータルに貼り付けます。
6. **手順**
   1. Azureポータルを開きます。
...
9. Intune管理者の**ユーザ名**と**パスワード**を入力します。
   - Azureアカウントにはグローバル管理者権限または限定管理者 + Intuneサービス管理権限が必要です。
   - Ivantiは、Intuneサービス管理権限のみのローカルAzureアカウントの作成を推奨しています。 IDプロバイダーに連携しているユーザアカウントはMicrosoftにサポートされていないためGraph APIでの認証ができません。

10. ****送信した日付が正しくない場合、エラーメッセージが表示されます。


## Office 365アプリ保護のポリシー

****Microsoft Graphの認証情報を構成した後、[アプリ] > [Office 365アプリ保護] を開き、各ユーザグループについてiOSまたはAndroidデバイスに対応する新しいOffice 365アプリ保護ポリシーを追加します。

ポリシーは **[アプリ] > [Office 365アプリ保護]** ページにリスト化されています。ページは **[アプリポリシー]** タブにあります。 ポリシーのリストは、最新のタイムスタンプ、プラットフォーム、割り当てられたアプリ、展開されているユーザグループなど、表形式の詳細情報を提供します。
...
**[アプリ] > [Office 365アプリ保護]** ページを開き、**[アプリ構成]** タブで各ユーザグループのOffice 365アプリ構成を追加、変更、削除します。 このアプリ構成で、管理者はキーと値のペアのリストを追加し、 1つ以上のOffice 365アプリに構成を割り当てることができます。 [アプリ構成] タブには、最新のタイムスタンプ、割り当てられたアプリ、展開のステータスなど、表形式で詳細情報が記載されています。
...
5. あるいは、**[アプリ配布]** または **[ユーザグループ配布] **タブをクリックします。 **[編集]** をクリックし、それらの設定だけを変更して **[保存]** をクリックします。
6. 構成の設定を変更します。
7. **[次へ]** をクリックします。
8. この構成を適用するアプリのリストを変更します。
9. **[次へ]** をクリックします。
10. この構成を適用するユーザグループを変更します。
11. **[完了]** をクリックします。
...
1. **[アプリ] > [Office 365アプリ保護]** を開きます。
2. **[アプリ構成]** をクリックします。
3. **[アクション]** 列で、削除したい構成名に対応する削除アイコンをクリックします。
4. **[はい]** をクリックして確定します。


##  Office 365 アプリを使用するコンプライアンス違反ユーザ

管理者は、コンプライアンス違反のあるユーザとデバイスのリストを確認できます。 このページを使用し、フラグのあるデバイスにOffice 365アプリがある場合はワイプしてください。
...
1. **[アプリ] > [Office 365アプリ保護]** を開きます。
2. ****[コンプライアンス違反ユーザ] をクリックします。
3. 以下のいずれかのアクションを実行します。
   - リストからユーザを選択し、**[Office 365アプリをワイプ]** をクリックします。
   - ユーザの名前をクリックし、コンプライアンス違反のアプリを持つデバイスのリストを表示します。 **[アクション]** 列で、特定のデバイスの **[Office 365アプリをワイプ]** アイコンをクリックします。
   - ユーザの名前をクリックし、コンプライアンス違反のアプリを持つデバイスのリストを表示します。 各デバイスの名前をクリックし、アプリとバンドルID/パッケージ名、フラグの理由を確認します。 **[Office 365アプリをワイプ]** をクリックします。
...
## Office 365アプリ保護を使用するユーザのアプリレポート

管理者は、以下のレポートのいずれかを選択し、Office 365アプリ保護を使用しているユーザのリストと関連情報を確認できます。
- アプリポリシーレポート
- アプリ構成レポート
- ワイプ要求


アプリレポートに記載されている情報には、バンドルID/パッケージ名、デバイス名、デバイスのタイプ、ポリシーまたは構成（デバイスに適用されている）、ステータス（同期している、同期しているが古い、同期していない）、最後のチェックイン時刻などがあります。 アプリレポートの情報をCSVファイルにエクスポートし、後で参照や分析を行うことができます。

ワイプ要求レポートに記載されている情報には、表示名、ユーザ名、デバイス名、デバイスのタイプ、ワイプステータス（ワイプ保留中またはワイプ完了）などがあります。
...
6. （任意）**[CSVにエクスポート]** をクリックし、レポートの内容をCSVファイルにダウンロードして、後で参照や分析を行うことができます。
