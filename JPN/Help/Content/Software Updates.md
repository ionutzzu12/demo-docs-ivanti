...
9. （任意）iOSデバイスの更新をスケジュールするには、**[予定時刻内に一度だけOS更新を送信]** オプションのチェックボックスを選択します。
10. **[次へ]** をクリックします。
11. **[この構成を有効化]** オプションを選択します。
12. 以下の配布オプションから1つ選択します。
    - すべてのデバイス
    - デバイスなし（デフォルト）
    - カスタム
...
- ``iOS 制限の enforcedSoftwareUpdateDelay を有効にして、ソフトウェア更新のデバイスでの手動スキャンによって、この構成でダウンロードされた特定のバージョンが削除されないようにします。
...
6. デバイスの更新のタイプを選択します。 各更新について、再起動を必要としない更新も選択可能です。
   - OS更新
   - 重大な更新
   - 構成データ更新
   - ファームウェア更新
   - ****管理者は、[重大ではない更新] を有効にすることにより、重大ではないmacOSの更新の管理（インストール/スケジュール）ができます。 このオプションは既存のテナントではデフォルトで無効になっており、必要に応じてアップグレード後に管理者が明示的に有効にする必要があります。 **OS アップデート**では、管理者はデバイスを特定のバージョンの macOS にアップデートできます。
すべての macOS アップデートは、次のアクションで構成できます。
     - デフォルト
     - 通知のみ
     - 後でインストール
     - 強制再起動をインストール
...
「後でインストール」オプションが選択されている場合にのみサポートされます。
...
- アクティブ時間の最大範囲 – 開始時間から数えて、ユーザがアクティブ時間を設定できる最大時間数を示すには、このオプションを有効にします。
- アクティブ時間中には更新通知なし – 更新通知オプションは以下のとおりです。
  - デフォルトのWindows更新通知
  - 再起動警告を除くすべての通知をオフ
  - 再起動警告を含むすべての通知をオフ
...
- ユーザによる「更新を一時停止」アクセスを無効化
- ユーザによるUXWUアクセスを無効化（Windows更新スキャン、ダウンロード、インストール）
