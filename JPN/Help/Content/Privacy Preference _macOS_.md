...
4. 構成の名前と説明を入力します。
5. ページ上に表示されたアプリケーションのいずれかに移動します。 [****](https://developer.apple.com/documentation/devicemanagement/privacypreferencespolicycontrol/services)関連情報は、Appleドキュメンテーションを参照してください。   1. macOS 10.14+の場合、構成可能なアプリケーションと設定は次のとおりです： 
   ...
   2. macOS 10.15+の場合、構成可能なアプリケーションと設定は次のとおりです： 
      - ファイルの使用 - ファイルプロバイダーによって管理されるファイルをユーザが使用していることを、ファイルプロバイダーアプリケーションが認識することを許可します。
      ...
      - ユーザのドキュメントフォルダーにあるファイルにアクセス - アプリケーションがユーザのドキュメントフォルダーにあるファイルにアクセスすることを許可します。
      - ユーザのダウンロードフォルダーにあるファイルにアクセス - アプリケーションがユーザのダウンロードフォルダーにあるファイルにアクセスすることを許可します。
