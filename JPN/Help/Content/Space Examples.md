...
ACME株式会社 は北米と欧州に事業所があります。 言語およびタイムゾーンの問題から、ACMEは米国内の管理者が北米のデバイスを管理し、ドイツ国内の管理者が欧州のデバイスを管理するようにしたいと考えています。

これらのスペースを設定するために、ACMEは次の変更を行いました。

1. 欧州のユーザ向けに Ivanti Neurons for MDM でユーザグループを作成。
2. 北米のユーザ向けの Ivanti Neurons for MDM 内にユーザグループを作成。
3. 以下のルールで欧州のスペースを作成：
4. 以下のルールで北米のスペースを作成：
...
以下のルールで北米 Android スペースを作成しました:
3. ユーザーグループ = 北米

OS = Android

各スペースのデバイス管理ロールを適切な管理者に割り当てました。
...
2. ユーザー名 = jdoe\\@acme.com
ユーザー名 = gkunz\\@acme.com
ユーザー名 = prizzo\\@acme.com
ユーザー名 = fvanhoff\\@acme.com
スペースを **スペース** ページのリストの先頭に移動しました。
3. そうしないと、Android デバイスを持つ経営幹部が間違った管理者になってしまうことになります。

スペースのデバイス管理ロールを特別な管理者に割り当てました。
