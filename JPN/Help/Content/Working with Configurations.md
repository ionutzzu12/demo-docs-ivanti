...
多くの [**タイプの構成**](\<./Configuration Types.md>) が利用可能です。 構成は次のような基本的カテゴリに分類されます。
- セキュリティ
- ユーザリソース
- エンタープライズネットワークアクセス
- セルラーネットワーク
...
**[構成]** ページには、すべての構成がリスト表示されます。 このリストを特定の構成に絞り込むには、OSおよび構成のタイプのフィルター（左ペイン）を使用します。 たとえばmacOSの構成のみ表示するようリストを絞り込むには、**[OS]** セクションで **[macOS]** を選択します。
...
リリース81から、グローバル管理者はスペース管理者に、すべてのデバイス向けおよびカスタム配布オプション向けの動的生成ID証明書の編集を委譲できます。
...
5. 構成の配布レベルを選択します。
   ...
   - 管理者はカスタム配布オプションを使用し、カスタム構成をデバイス、デバイスグループ、ユーザ、ユーザグループに配布できます。 ユーザまたはユーザグループへの構成の割り当てまたは配布は、以下の構成では使用できません。 
...
5. 以下の配布オプションから1つ選択します。
   - **すべてのデバイス**。 以下のオプションから1つ選択してください：
     ...
     - **他のデバイススペースにあるすべてのデバイスに、最低優先度で適用する**。 （これは、Wi-Fi構成の場合にのみ適用されます）。
       - **[スペース管理者に配布の編集を許可]** のチェックボックスを選択すると、委譲スペース管理者が特定のスペースの配布を編集できるようになります。
...
6. スペースに関係なく、動的生成ID証明書はすべてのスペースに対して構成し、すべてのデバイスに配布し、他のデバイススペースのすべてのデバイスに適用できます。
7. **[完了]** をクリックします。
...
6. 1つの構成をプッシュするには、**[アクション]** 列から **[プッシュ]** をクリックします。
...
5. ************1つの構成を除外するには、[アクション] 列から [除外] をクリックします。 選択した構成が、[除外された構成] タブにリスト表示されます。
...
1. **[構成]** に進みます。
2. 既存の構成を選択します。
3. ア**クション **プルダウン メニューをクリックし、 エ**クスポートを**クリックします。
構成の名前とタイムスタンプが付いたファイル \\yyyymmdd.yaml がダウンロードされます。
...
- ID\\証明書\\DG
- 制限
- ANDROID\\WORK\\DEVICE\\OWNER\\LOCKDOWN
- パスコード
- ホーム\\画面\\レイアウト
- ANDROID\\ADVANCED\\PASSCODE\\AND\\LOCK\\SCREEN
- IOS\\アプリ\\コントロール
- SCEP
- ANDROID\\SAMSUNG\\SAFE\\ロックダウン
- ANDROID\\WORK\\DEVICE\\OWNER\\WITH\\WORK\\PROFILE\\LOCKDOWN
- ANDROID\\WORK\\ロックダウン
- デバイス\\名前
- モバイル脅威防御
- プライバシー
...
- ANDROID\\アプリ\\接続
- APPCONNECT\\デバイス\\設定
- ANDROID\\WORK\\DEVICE\\OWNER\\SYSTEM\\UPDATE
- ドメイン
- エンドユーザープライバシー
- 交換
- MI\\クライアント\\プライバシー
...
- WINDOWS\\デバイス\\管理者\\制限
- テレコム\\設定
- IDENTITY\\CERTIFICATE\\UP
- IDENTITY\\CERTIFICATE\\DC
- WI-FI
- 通知
- ウェブ\\コンテンツ\\フィルター
- WINDOWS\\AI\\管理
...
4. 構成の詳細を確認し、次のいずれかの配布概要オプションを選択します。

配布概要はすべての構成で利用できるわけではありません。
   - **他のスペースに適用しない**。
   - **他のスペースのデバイスに適用します** 配布はデフォルトで「デバイスなし」に設定されています。
...
## [****](#)自社開発アプリ更新のスケジュール
 ...
 は、デバイスがチェックインしたときに自社開発アプリケーションを自動的に更新します。 管理者は、サーバのタイムゾーンに基づいて、社内アプリケーション更新をスケジュールできます。 アプリの更新は、スケジュールされた時間帯にデバイスがチェックインしたときのみ実行されます。 アプリ更新のスケジュールはデフォルトで無効化されています。 
...
8. **[次へ]** をクリックします。
9. 必要なユーザ/デバイスグループを選択し、**[この構成を有効化]** のチェックボックスをクリックします。
10. **[完了]** をクリックします。 構成が適用されると、所定のスケジュールでのみアプリが更新されます。


[構成] ページが表示されない場合、必要な権限を持っていない可能性があります。 次のいずれかの [**ロール**](\<./User Roles.md>) が必要です。
