...
- iOS/macOSアプリ管理設定の変更
- App Storeのデフォルト地域の設定
- iOSアプリ更新の有効化/無効化
- アプリの評価とレビューの有効化/無効化
- iOS/macOSの「Appとブック」sToken（ライセンス：[%=PrimaryMI.license3%]）のアップロードまたは更新
...
- iOS および macOS の Apps and Books トークンをアップロード ( ライセンスが必要)
...
ユーザのデバイスの場合、ユーザがアプリカタログメニューの **[強制チェックイン]** ボタンをクリックしてデバイスをチェックインすると、アプリ更新、その他の構成や更新が行われます。 

このような設定により、エンドユーザはアプリの更新時期を選択できます。
...
これにより、ユーザはアプリを評価してレビューを書き、他のユーザに読んでもらうことができます。

1. **[エンドユーザのアプリカタログで評価とレビューを有効化]** を選択するか選択解除します。
2. **[保存]** をクリックします。
...
## [****](#)iOS/macOSの「Appとブック」sToken（ライセンス：）のアップロードまたは更新
...
の前のリリースで、個々のユーザに「Apps and Books」トークンが予約されている場合は、それらのユーザのトークンがまだ予約されているかどうか確認し、必要ならば再度予約する必要があります。

## [****](#)Ivanti Neurons for MDM サービスからのiOS/macOSの「Appとブック」sTokenの削除

ユーザが必要としなくなったアプリは、無効化し、必要に応じて再び割り当てることができます。 アプリがiOS/macOS向けMDMによってマネージドアプリとして展開されていた場合は、アプリと全データを即座に削除できます。

1. 削除するアプリを選択します。
2. **[削除]** をクリックします。

    警告ダイアログが表示されます。
3. ユーザに30日間の猶予を与え、以下を行わせることも可能です。 
   - 自分のデータの保存
   - 個人用として同じアプリの購入
...
**[カタログ設定]** ページでタスクを実行できない場合、必要な権限を持っていない可能性があります。 次の [**ロール**](\<./User Roles.md>) が必要です。
