...
The Spaces page and the associated options are hidden for converged tenants who have access to both Ivanti Neurons for UEM and .
...
When an administrator logs in to the Admin Portal on a tenant with at least a single Delegated Space, the administrator is presented with the Admin Portal login promo pop-up. The promo pop-up will not be displayed after delegated space creation and during user login if a delegated space is already created.
