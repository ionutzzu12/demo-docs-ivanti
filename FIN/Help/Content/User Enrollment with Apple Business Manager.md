...
- Unsupervised devices with iOS 13.0 through the latest version as supported by .
- Devices with macOS 10.15or supported newer versions .
...
- An unsupervised device with iOS 13.0 through the latest version as supported by or a device with macOS 10.15or supported newer versions .
...
- Managed Apple ID - Managed Apple ID to be associated with each enrolled device.
  ...
  - Managed Apple IDs were first utilized by Apple School Manager and are now utilized by Apple Business Manager for User Enrollment.
    ...
    If they are different, a notification is displayed in the Admin Portal when the license allocation fails for an app.
...
- User Enrollment is supported via Go for iOS client and iReg. Apple no longer supports User Enrollment through Ivanti Go for iOS and iReg. Account driven User Enrollment is only available in the Work or School account of the device.
...
- If you Re-enroll a device from Go for iOS client, the enrollment type will be the same as the type during the device registration irrespective of the change in the enrollment type in . For example, if a device was user enrolled, change the type to Device Enrollment in , and Re-enroll the device from the Go client, the device will still be user enrolled and not device enrolled.
...
The following list indicates what a server can do in a standard MDM enrollment, but will not be able to do in User Enrollment mode.
...
User Enrollment is supported for devices with macOS 10.15or supported newer versions .
...
- App dependency and behavioral changes in macOS User Enrollment Enrolled devices.
  ...
  - Installed apps are indicated as Requested Apps in **Devices > App Inventory** page as the macOS User Enrollment Enrolled devices do not notify the server whether the apps are installed or not installed in the inventory report.
...
- Not all configurations, policies, and actions are allowed. See full list of configurations and policies listed following this procedure.
  ...
  - Similarly, unsupported admin device actions will be informed in UI.
  - Unsupported reports will not be sent by .
...
| Apply supervised restrictions                                                                                              | ![](Resources/Images/X.svg)     | ![](Resources/Images/X.svg)     | ![](Resources/Images/Check.svg)<br />(Supervised devices only) |
...
- You must request and install an Apple [**MDM certificate**](<./Install MDM Certificate.md>) to manage iOS devices.
...
1. Go to Users **>** [**User Groups**](<./User Groups.md>).
...
1. Go to **Users >** [**User Settings**](<./User Settings.md>).
...
- As a prerequisite, set up a connector to access [**LDAP**](<./ Configuring LDAP server.md>) resources.
...
- You can import users from LDAP and invite them for User Enrollment. The imported LDAP users will have their Managed Apple IDs synced with , which is a requirement for User Enrollment.
...
As a prerequisite, connect with Microsoft Entra ID (Entra ID).

You can invite Entra ID users for User Enrollment. The imported Entra ID users will have their Managed Apple IDs synced with , which is a requirement for User Enrollment.

Procedure

1. Go to **Admin >&#xA0;**[**Entra ID User Source**](<./Connect with Entra ID User Source.md>).
2. Edit the settings.
...
6. Select **Automatically invite users imported from Entra ID**. Users imported from Entra ID to are automatically invited to register via email.
7. Click **Save**.
